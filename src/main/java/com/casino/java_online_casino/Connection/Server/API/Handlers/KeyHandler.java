...
public class KeyHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        LogManager.logToFile("[DEBUG] Otrzymano zapytanie HTTP: " + exchange.getRequestMethod() + " " + exchange.getRequestURI());
        ...
        try (InputStreamReader reader = new InputStreamReader(exchange.getRequestBody(), StandardCharsets.UTF_8)) {
            ...
            LogManager.logToFile("[DEBUG] Odebrano body żądania: " + requestBody);
            ...
            LogManager.logToFile("[ERROR] Wyjątek podczas obsługi KeyHandler: " + e.getMessage());
    ...
    private boolean isPostMethod(HttpExchange exchange) {
        if (!exchange.getRequestMethod().equalsIgnoreCase("POST")) {
            ...
            LogManager.logToFile("[DEBUG] Nieobsługiwany typ metody: " + exchange.getRequestMethod());
            return false;
        }
        return true;
    }
    ...
    private boolean hasClientPublicKey(JsonObject requestBody) {
        if (!requestBody.has(JsonFields.CL_PUBLIC_KEY)) {
            ...
            LogManager.logToFile("[DEBUG] Brak pola '" + JsonFields.CL_PUBLIC_KEY + "' w żądaniu");
            return false;
        }
        return true;
    }
    ...
    private void handleKeyExchange(HttpExchange exchange, JsonObject requestBody) throws IOException {
        ...
        LogManager.logToFile("[DEBUG] Klucz publiczny klienta (Base64): " + clientPublicKeyBase64);
        ...
        LogManager.logToFile("[DEBUG] Klucz klienta zaimportowany do KeyManagera.");
        System.out.println("[DEBUG] Klucz AES: " + session.getKeyManager().exportAesKey());
        LogManager.logToFile("[DEBUG] Klucz AES: " + session.getKeyManager().exportAesKey());
        ...
        LogManager.logToFile("[DEBUG] Wygenerowany klucz publiczny serwera (Base64): " + serverPublicKeyBase64);

        UUID uuid = session.getUuid();
        System.out.println("[DEBUG] Wygenerowany UUID sesji: " + uuid);
        LogManager.logToFile("[DEBUG] Wygenerowany UUID sesji: " + uuid);

        String token = session.getNewToken();
        System.out.println("[DEBUG] Wygenerowany token JWT: " + token);
        LogManager.logToFile("[DEBUG] Wygenerowany token JWT: " + token);
        ...
        LogManager.logToFile("[DEBUG] Sesja zapisana w KeySessionManager pod UUID: " + uuid);
    }
    ...
    private void logAndSend(HttpExchange exchange, int statusCode, JsonObject body) throws IOException {
        ...
        LogManager.logToFile("[DEBUG] Wysyłany JSON: " + body);
