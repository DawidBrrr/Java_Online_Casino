...
public class PokerRoomManager {
    ...
    private PokerRoomManager() {
        ...
        LogManager.logToFile("[DEBUG POKER ROOM MANAGER] Utworzono nową instancję managera");
    ...
    public PokerRoom createRoom(PokerTCPClient tcpClient) {
        ...
        LogManager.logToFile("[DEBUG POKER ROOM MANAGER] Próba utworzenia nowego pokoju dla klienta: " + tcpClient);
        if (tcpClient == null) {
        ...
        LogManager.logToFile("[DEBUG POKER ROOM MANAGER] Utworzono pokój o ID: " + room.getRoomId() + ". Aktywne pokoje: " + rooms.size());
        return room;
    }

    public Optional<PokerRoom> getRoom(String roomId) {
        System.out.println("[DEBUG POKER ROOM MANAGER] Szukam pokoju o ID: " + roomId);
        LogManager.logToFile("[DEBUG POKER ROOM MANAGER] Szukam pokoju o ID: " + roomId);
        ...
        LogManager.logToFile("[DEBUG POKER ROOM MANAGER] " + (room != null ? "Znaleziono pokój" : "Nie znaleziono pokoju"));
        return Optional.ofNullable(room);
    }

    public void removeRoom(String roomId) {
        System.out.println("[DEBUG POKER ROOM MANAGER] Próba usunięcia pokoju o ID: " + roomId);
        LogManager.logToFile("[DEBUG POKER ROOM MANAGER] Próba usunięcia pokoju o ID: " + roomId);
        PokerRoom room = rooms.remove(roomId);
        if (room != null) {
            room.closeRoom();
            System.out.println("[DEBUG POKER ROOM MANAGER] Usunięto pokój o ID: " + roomId);
            LogManager.logToFile("[DEBUG POKER ROOM MANAGER] Usunięto pokój o ID: " + roomId);
        }
    }
    ...
    public void cleanInactiveRooms() {
        ...
        LogManager.logToFile("[DEBUG POKER ROOM MANAGER] Rozpoczynam czyszczenie nieaktywnych pokoi. Przed: " + beforeSize);
        rooms.entrySet().removeIf(entry -> {
            ...
            if (inactive) {
                ...
                LogManager.logToFile("[DEBUG POKER ROOM MANAGER] Znaleziono nieaktywny pokój: " + entry.getKey());
            }
            return inactive;
        });
        System.out.println("[DEBUG POKER ROOM MANAGER] Zakończono czyszczenie. Po: " + rooms.size());
        LogManager.logToFile("[DEBUG POKER ROOM MANAGER] Zakończono czyszczenie. Po: " + rooms.size());
    }

    public Map<String, PokerRoom> getActiveRooms() {
        System.out.println("[DEBUG POKER ROOM MANAGER] Liczba aktywnych pokoi: " + rooms.size());
        LogManager.logToFile("[DEBUG POKER ROOM MANAGER] Liczba aktywnych pokoi: " + rooms.size());
        return rooms;
    }
